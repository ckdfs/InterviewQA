...
作答或录音被报错打断时，报错行旁边要给一个可点的下一步。用户不该被要求把刚才那段话
重说一遍，也不该自己去猜下一步该做什么。

**事件契约**：`error` 事件可以带一个 `retry` 描述符，渲染层据此决定挂不挂重试按钮。
不带 `retry` 的 `error` 就是一条纯提示，不出现按钮。

| `retry.kind` | 触发场景 | 重试做什么 |
| --- | --- | --- |
| `answer` | 缺少 DeepSeek Key、调用 DeepSeek 失败 | 用同一个问题重新生成回答 |
| `record` | 打不开设备、没拿到音频轨、音频通道未就绪、转写全部失败 | 按当前音源重新开始录音 |

**按钮形态**：28 像素的红色圆形图标按钮（`.retry-btn`，环形箭头由 `createRetryIcon()`
用 SVG 画），挂在报错气泡右侧并垂直居中。做成纯图标是为了不占对话的视觉重量，
代价是它没有文字，因此必须同时给 `title` 与 `aria-label`；冒烟测试会断言这两项、
图标是否存在、尺寸不超过 32 像素、以及底色确实是红色。

三条容易踩空的约定：

- **失败也要发 `answer:done`**。渲染层的回答气泡靠它清掉，`state.answering` 也在这条路径上
  收回。缺了它界面会一直挂着输入指示，而且 `answering` 永远为真，之后既不能提问也不能重试。
- **重试是同一次提问的再次尝试，不是新一轮提问**。渲染层用 `pendingRetry` 让 `answer:start`
  跳过插入提问气泡，重跑之后对话里不会出现两条一模一样的问题。主进程没接住这次重试时必须
  把标记清掉，否则下一次提问不会出现在对话里。
- **一个字都没生成就失败时不留空气泡**。`answer:done` 收到 `failed` 且气泡为空就把它移除，
  否则界面上留一个空框，看起来像还在生成。

**转写失败与没录到声音要分开报**。片段全部失败时问题在转写服务这一侧（Key、额度、网络），
报成「没识别到内容」会把用户引去检查电脑有没有在播放，方向是错的。因此 `maybeFinalize`
按 `chunkFailures` 与 `chunkSeq` 的关系区分这两类，并把首个错误原因带进报错行。
...
- **环境变量是运行期覆盖，绝不落盘**：`DEEPSEEK_API_KEY` / `MIMO_API_KEY` 在本次运行内
  优先于文件，便于 CI 与临时启动。`load()` 会把这些路径记进 `envOverridden`，`persist()`
  写盘前用 `diskData()` 把它们还原成磁盘上的原值（磁盘上本来没有的字段则从写盘内容里删掉）。
  用户在设置里显式改过的字段会从 `envOverridden` 中移除，此时由用户的值落盘。

  这一条不是洁癖：`persist()` 写的是整份合并后的配置，所以只要环境变量的值进了内存配置，
  任何一次保存——哪怕只是记一下引导已走完——都会把它写进 `config.json`，静默顶掉用户
  原先填在里面的 Key。这类损失没有任何提示，`npm run test:config` 专门盯住它。

增写盘内容的两条既有行为，手改配置前要知道：

- 写盘写的是**合并后的完整骨架**，不是只写改过的那几个字段；因此手写进 `config.json` 的
  未知字段会在下一次保存时消失。
- 字段是**嵌套 JSON**（`{"answer": {"maxChars": 260}}`）。上表的点号写法只表示层级，
  写成扁平的 `"answer.maxChars": 260` 不会被读到，也不会报错。
...
npm run test:config   # 配置层：增量式合并、白名单校验、环境变量只在运行期生效且从不落盘
npm run test:update   # 自动更新：签名判定、更新方式、状态迁移、安装条件
npm run test:ui       # 界面冒烟：元素引用、引导向导、音源切换、连接自检、报错重试、设置面板、保存字段白名单
```
...
- `test:ui` 的桩状态要跟着保存请求变化（`applyPatch`），否则「保存后回到上一步」这类正常流程
  会在冒烟里假失败。
- `test:ui` 用 `webContents.send` 直接投递主进程事件来驱动对话：报错重试那一组就是这样造出
  「开始作答 → 报错 → 收尾」三轮事件的，不依赖真实网络。
- 断言脚本里点不到元素会让 `executeJavaScript` 抛错。这类失败由 `report()` 统一收成一条问题，
  **不要让整轮自测卡在未处理的 Promise 上**——那样日志里只剩一句看不出原因的堆栈。
- **新断言要故意改坏一次再跑**，确认它真的会失败。重试那一组都这样验过：去掉重试按钮、
  让 `pendingRetry` 失效、保留失败后的空气泡；按钮形态改成文字按钮或放大到 40 像素、
  换成非红色，也都会如期报错。
- 本地跑界面冒烟：
...
- **macOS 更新检查的网络依赖**：公司网络拦截 `github.com` 时检查更新会失败，这属于网络环境
  问题，不必改代码。
- **开发模式下配置就在项目目录**：`config.json` 属于本人真实配置。验证失败路径时优先用
  `DEEPSEEK_API_KEY=<无效值> node scripts/launch.js ...` 这样的临时注入，验证完把变量清掉；
  跑任何会触发保存的流程前，先记下 `shasum` 与 Key 尾部，跑完对比一次。
- **环境变量会跨命令残留**：`DEEPSEEK_API_KEY=... cmd` 这种前置写法在本机 shell 里可能留到
  后续命令，导致本该走成功路径的验证悄悄变成失败路径。要跑干净的一轮就用
  `env -u DEEPSEEK_API_KEY -u MIMO_API_KEY`。
- **BSD grep 不支持 `\|` 交替**：`grep "a\|b" file` 会静默返回空，容易误判成「没有残留」。
  多模式匹配用 `grep -E "a|b"`，或改用 rg。
