...
| 能力 | Windows | macOS |
| --- | --- | --- |
| 录制系统声音 | WASAPI 回环，开箱可用 | 走 Screen & System Audio Recording 权限体系，需 macOS 13+ 并授予「屏幕录制」权限 |
| 手动输入提问 | 可用 | 可用 |
| 本地 Whisper 兜底 | 需自建 `.venv` | 同左 |

Electron 39 起 Chromium 默认改用 CoreAudio Tap 采集桌面音频。该路径创建失败时只返回一条静音且已结束的音频轨，官方文档说明不会有任何警告或错误，表现为「录完什么都没识别到」。应用固定切回 Screen & System Audio Recording 权限体系，并在拿到音频轨后校验它处于 live 状态，遇到静音轨会立即提示，不会录下一段空白。
...
- **macOS 录到的是静音**：应用会在开始时校验音频通道，若提示「系统音频通道没有真正启动」，重启应用后重试；仍未恢复就回到系统设置确认本应用的屏幕录制权限处于勾选状态。
